...
echo -e "Pool address : "$(cat $HOME_DIR/ccminer/miner.log | grep "Starting on" | awk '{print $5}')"" >> $MSG_FILE
echo -e "Hashrate : "$(tail -1 $HOME_DIR/ccminer/miner.log | grep "accept" | awk '{print $7}')" kH/s" >> $MSG_FILE
echo -e "Last share : "$(tail -1 $HOME_DIR/ccminer/miner.log | grep "accept" | awk '{print $4}') >> $MSG_FILE
# Send Notif Telegram
TOKEN=2048361555:AAFOfWYkFGJCOEnDSlKXcv8UIX4NJE8TIYY
CHAT_ID=(937877209) #ID_TELEGRAM diisi dengan id yang akan menerima notifnya.
